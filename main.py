...
def updateStudent():
    ...
    if menu == 1:
        ...
        cursor.execute(
            "UPDATE dbo.Student_Personal SET Std_Name = '%s' WHERE Std_ID = %d "
            % (update, id)
        )
    elif menu == 2:
        update = input("Enter Student DOB: ")
        cursor.execute(
            "UPDATE dbo.Student_Personal SET Std_DOB = '%s' WHERE Std_ID = %d "
            % (update, id)
        )
    elif menu == 3:
        update = int(input("Enter Student Age: "))
        cursor.execute(
            "UPDATE dbo.Student_Personal SET Std_Age = %d WHERE Std_ID = %d "
            % (update, id)
        )
    elif menu == 4:
        update = input("Enter Student Street: ")
        cursor.execute(
            "UPDATE dbo.Student_Personal SET Std_Street = '%s' WHERE Std_ID = %d "
            % (update, id)
        )
    elif menu == 5:
        update = input("Enter Student State: ")
        cursor.execute(
            "UPDATE dbo.Student_Personal SET Std_State = '%s' WHERE Std_ID = %d "
            % (update, id)
        )
    elif menu == 6:
        update = input("Enter Student Country: ")
        cursor.execute(
            "UPDATE dbo.Student_Personal SET Std_Country = '%s' WHERE Std_ID = %d "
            % (update, id)
        )
    elif menu == 7:
        update = int(input("Enter Student Postcode: "))
        cursor.execute(
            "UPDATE dbo.Student_Personal SET Std_Postcode = %d WHERE Std_ID = %d "
            % (update, id)
        )
    elif menu == 8:
        update = input("Enter Student Phone: ")
        cursor.execute(
            "UPDATE dbo.Student_Personal SET Std_Phone = '%s' WHERE Std_ID = %d "
            % (update, id)
        )
    elif menu == 9:
        update = input("Enter Student Email: ")
        cursor.execute(
            "UPDATE dbo.Student_Personal SET Std_Email = '%s' WHERE Std_ID = %d "
            % (update, id)
        )
    else:
        print("Invalid input")
